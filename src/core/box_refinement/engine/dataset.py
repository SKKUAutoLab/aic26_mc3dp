"""Dataset scanning + single-frame extraction for the AICity MTMC dataset.

Layout per scene (some files optional, especially for ``test``)::

    {root}/{split}/{scene}/
        calibration.json
        videos/Camera_XXXX.mp4
        depth_maps/Camera_XXXX.h5   (train/val only)
        ground_truth.json           (train/val only)
        map.png
"""
