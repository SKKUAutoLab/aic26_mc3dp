...
class ClassLabels:
	...
	def ids(
		...
		Returns:
			labels_ids (list):
				List of all labels' ids at `key`, excluding those with negative or
				max keys if specified.
		"""
		labels_ids = []
		for label in self.class_labels:
	...
	def get_id(
			self, key: str = "name", value: Union[int, str, None] = None
	) -> Optional[int]:
	...
	def get_color(
		...
		"""Get the class_label's color based on the given (`key`, `value`) pair."""
		class_label = self.get_class_label(key=key, value=value)
		return class_label["color"] if class_label is not None else None
